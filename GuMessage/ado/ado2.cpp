...
#ifdef _LOG4CPLUS_
	g_Logger.Debug(__FILE__,__LINE__,ErrorStr.GetBuffer());
#endif
	#ifdef _DEBUG
...
{	
	...
	try
	{
		...
		if(bIsSelect || nOption == openQuery || nOption == openUnknown)
			m_pRecordset->Open((LPCTSTR)m_strQuery.GetBuffer(0), _variant_t((IDispatch*)mpdb, TRUE),
...
BOOL CADORecordset::Open(LPCTSTR lpstrExec, int nOption,CDataTable* pDt/*=NULL*/)
{
...
BOOL CADORecordset::Open(LPCSTR lpstrExec, int nOption,CDataTable* pDt/*=NULL*/)
{
	ASSERT(m_pConnection != NULL);
	ASSERT(m_pConnection->GetState() != adStateClosed);
	return FALSE;
...
{	
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			val /= pow(10.0, (int)vtFld.decVal.scale); 
			break;
		case VT_UI1:
			val = vtFld.iVal;
			break;
...
BOOL CADODatabase::Execute(LPCSTR lpstrExec)
{
	return FALSE;
}
...
BOOL CADORecordset::GetFieldInfo(ADODB::FieldPtr pField, CADOFieldInfo* fldInfo)
{
	memset(fldInfo, 0, sizeof(CADOFieldInfo));
#ifdef _UNICODE
	LPCTSTR wsName =(LPCTSTR)pField->GetName();
	//USES_CONVERSION;
	//strcpy(fldInfo->m_strName, W2A(wsName));
	_tcscpy_s(fldInfo->m_strName, wsName);
#else
	strcpy_s(fldInfo->m_strName, 32,(LPCTSTR)pField->GetName());
#endif
...
BOOL CADORecordset::GetChunk(ADODB::FieldPtr pField, CString& strValue)
{
	CString str = _T("");
...
BOOL CADORecordset::GetChunk(ADODB::FieldPtr pField, LPVOID lpData)
{
	long lngSize, lngOffSet = 0;
	_variant_t varChunk;    
	UCHAR chData;
...
CDataTable* CADORecordset::GetDt(CDataTable* dt)
{
	CDataColumn* pColumn = NULL;
	CDataTable* pTable=dt;//new CDataTable;
	pTable->SetDs(this);	
	for (int i=0,sz=GetFieldCount();i<sz;i++)
	{
		CADOFieldInfo aAdoFileInfo;
		GetFieldInfo(i,&aAdoFileInfo);
		pColumn = new CDataColumn();
		//pColumn->m_ColumnName = aAdoFileInfo.m_strName;
		_tcscpy_s(pColumn->m_ColumnName ,32,aAdoFileInfo.m_strName);
		pTable->Columns.push_back(pColumn);
	}




	for(int i=0,sz=GetRecordCount();i<sz;i++)
	{
		CDataRow* pDr = pTable->NewRow();
		ASSERT(pDr);
		for (int j=0,sz=GetFieldCount();j<sz;j++)
		{
			CDataColumn* pColumn=NULL;
			pColumn = ((CDataColumn*)pDr->m_pColumns[j]);
			GetFieldValue(j,pColumn->value );
		}

		pTable->Rows.push_back(pDr);
...
BOOL CADOParameter::GetValue(double& dbValue)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			dblVal /= pow(10.0, (int)vtVal.decVal.scale); 
			break;
		case VT_UI1:
			dblVal = vtVal.iVal;
			break;
...
BOOL CADOParameter::GetValue(CString& strValue, CString strDateFormat)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			val /= pow(10.0, (int)vtVal.decVal.scale); 
			strVal = DblToStr(val);
			}
			break;
		case VT_DATE:
